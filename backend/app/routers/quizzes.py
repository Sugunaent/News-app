...
def _extract_translation(translations: list[dict], fallback_lang: str = "en") -> str | None:
    if not translations:
        return None

    for item in translations:
        if item.get("language_code", "").lower() == fallback_lang.lower():
            return item.get("question_text") or item.get("option_text")

    first_item = translations[0]
    return first_item.get("question_text") or first_item.get("option_text")


@router.get("/article/{article_id}", response_model=QuizResponse)
async def get_article_quiz(
    ...
    # 2. Fetch questions with translations
    try:
        questions_res = (
            client.table("quiz_questions")
            .select("id, quiz_id, display_order, quiz_question_translations(language_code, question_text)")
    ...
    # 3. Fetch options with translations if questions exist
    options_by_question: dict[str, list] = {}
    if question_ids:
        try:
            options_res = (
                client.table("quiz_options")
                .select("id, question_id, display_order, quiz_option_translations(language_code, option_text)")
                .order("display_order")
                .execute()
            )
    ...
    # 4. Map DB data to schema
    formatted_questions = []
    for q in questions_data:
        q_text = _extract_translation(q.get("quiz_question_translations", []))
        if not q_text:
            continue

        raw_options = options_by_question.get(q["id"], [])
        formatted_options = []
        for opt in raw_options:
            opt_text = _extract_translation(opt.get("quiz_option_translations", []))
            if opt_text:
...
async def submit_quiz_attempt(
    ...
    try:
        ...
        raise NotFoundError("Quiz question not found") from exc
    ...
    try:
        ...
        raise NotFoundError("Quiz option not found") from exc
...
# ---------------------------------------------------------
    ...
    return QuizSubmitResponse(
        attempt=QuizAttemptResponse(
            question_id=raw_attempt["question_id"],
            selected_option_id=raw_attempt["selected_option_id"],
            is_correct=raw_attempt["is_correct"],
            created_at=raw_attempt["created_at"],
        )
    )